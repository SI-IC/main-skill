...
test("findUp возвращает null если файл не существует", () => {
  ...
  const hit = af.findUp(path.join(root, "a"), [
    "какой-нибудь-несуществующий.lock",
  ]);
...
test("prettierInstaller: bun.lockb → bun", () => {
  ...
  assert.equal(
    af.prettierInstaller(path.join(root, "app.ts")),
    "bun add -d prettier",
  );
});
...
test("prettierInstaller: pnpm-lock.yaml → pnpm", () => {
  ...
  writeFile(root, "pnpm-lock.yaml", "");
  assert.equal(
    af.prettierInstaller(path.join(root, "src/app.ts")),
    "pnpm add -D prettier",
  );
});
...
test("pythonInstaller: uv.lock → uv", () => {
  ...
  assert.equal(
    af.pythonInstaller(path.join(root, "main.py")),
    "uv add --dev ruff",
  );
});
...
test("findLocalBin: walkUp не выходит за project-root marker", () => {
  ...
  assert.equal(
    found,
    null,
    "walkUp должен остановиться на project-root, не дойти до evil-родителя",
  );
});
...
test("findLocalBin: symlink-бинарь, ведущий за пределы project-dir, отклоняется", () => {
  ...
  fs.mkdirSync(path.join(root, "project", "node_modules", ".bin"), {
    recursive: true,
  });
  ...
  assert.equal(
    found,
    null,
    "symlink, ведущий за пределы project-dir, не должен выбираться",
  );
});
...
test("isProjectRoot: распознаёт стандартные маркеры", () => {
  ...
  for (const m of af.PROJECT_ROOT_MARKERS) {
    ...
    assert.ok(
      af.isProjectRoot(dir),
      `${m} должен распознаваться как project-root marker`,
    );
  }
});
...
test("main: MAIN_SKILL_OFF=1 → no-op даже когда без него был бы additionalContext", () => {
  ...
  const f = writeFile(root, "src/app.ts", "export const a = 1;\n");
  assert.notEqual(
    runMain({ tool_name: "Edit", tool_input: { file_path: f } }),
    "",
  );
  const out = runMain(
