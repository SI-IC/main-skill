...
## Комментарий — исключительный случай

Комментарий живёт до первой правки рядом, после чего врёт, а читатель верит ему, а не коду. Пиши его только там, где без него высок риск регресса при следующей правке, и начинай с «Не менять, потому что …» — предельно кратко, одной мыслью, для себя-следующего: `// Не менять, потому что statSync без isFile-guard виснет на FIFO`. Остальные — удаляй, встречая. Что «объяснял» комментарий, выражай кодом: имя функции вместо заголовка секции, именованная константа вместо пояснения к числу, отдельная функция вместо блока-«шага».

Не комментарии и правилу не подчиняются: функциональные директивы (`eslint-disable`, `# type: ignore`, `# noqa`, shebang), лицензионные и `@generated` заголовки, docstring / JSDoc с тегами над публичной сущностью — это API-документация.

Enforcement: PreToolUse `comment-guard.js`. Проект требует комментариев по конвенции → `MAIN_SKILL_COMMENT_CHECK=0`.

## CLAUDE.md — только невыводимое, ≤ 40 KB

CLAUDE.md грузится целиком в каждую сессию, вытесняя рабочий контекст. Держи в нём только то, что не выводится ни из кода, ни из тестов, ни из хуков: команды и воркфлоу, связи модулей, конфиг-квирки, осознанные компромиссы. Правило, которое можно утверждать тестом или проверять хуком, — принадлежит тесту или хуку; нет их, но они уместны → создай их вместо абзаца в доке. Инвариант «почему именно так» — рядом с кодом как `Не менять, потому что …`. Историю правок хранит git.

Формулировки — максимально краткие, для себя-следующего, а не для презентации. Дописываешь → сперва сокращай существующее. Enforcement: PreToolUse `claudemd-guard.js` (net-прирост + жёсткий кап 40 KB, `MAIN_SKILL_CLAUDEMD_MAXBYTES`).
...
**Что observable-правкой не считается:** доки, планы, задачники — `*.md` / `*.rst` / `*.txt` / `docs/**`. Ревью-агентов на них не запускай даже при Write на сотни строк: ревьюить нечего — нет ни диффа кода, ни поверхности для OWASP. Порог считает только код; правишь план вместе с кодом → ритуал требует код, не план.
...
**Премортем по плану — в `## Риски`, не в чат.** Внутрифазные риски покроет сама фаза (правит код → триггер N в её сессии). Автор плана обязан закрыть то, чего не увидит ни одна фаза: каждая фоновая сессия читает только преамбулу и своё тело, граф целиком был в контексте один раз — при планировании. Гипотезы про стыки фаз (контракт отдан не в том формате, что ждёт downstream; `deps` пропускают реальную зависимость; необратимый шаг помечен `autonomy=auto`; слабый verify-гейт пропускает дефект дальше) пиши в `## Риски` тем же форматом `вход/состояние → наблюдаемый отказ → решение` — эту секцию executor перечитывает на старте каждой фазы, а блок в чате умрёт вместе с сессией планирования.
