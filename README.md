...
The `verify-changes.js` Stop hook blocks "done" claims until tests are paired, docs are updated, lint is green, edge-cases are declared, a premortem block precedes non-trivial changes (trigger N: ≥3 specific "what breaks in prod" hypotheses — `input → observable failure → fix`, each with a concrete number / error code / identifier), and self-review is performed (three parallel reviewers: code, security, premortem lens). Both thresholds count code lines only — edits to docs, plans and task lists (`*.md`, `docs/**`) never arm N or J. It auto-detects test pairs across stacks (pnpm/yarn/cargo/go monorepos; Jest/Vitest/RSpec/PHPUnit/JUnit/Swift conventions) and skips files that aren't unit-testable (migrations, seeders, fixtures, locales, `*.d.ts`, `*.generated.*`, framework configs, type-only TS, `@generated`-headed files).
...
- `MAIN_SKILL_REVIEW_MODEL_CAP=0` — lift the opus ceiling on review subagents in J. By default a review call whose effective model (the model actually executed per `<session>/subagents/agent-*.jsonl`, else explicit `model`, else `message.model` of the assistant entry that made the call) is fable/mythos blocks the Stop: in a fable session security-review must be dispatched with `model: "opus"`. Use the opt-out when the model is forced externally and the subagent transcript is unavailable.
