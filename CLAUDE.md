...
- **Модель premortem-линзы гейтится** (`weak-edge-model` в J): `findReviewAgentCalls` отдаёт `edgeModel` — `input.model` ПОСЛЕДНЕГО premortem-вызова (его находки идут в триаж), `isWeakPremortemModel` матчит алиас `haiku` целиком ИЛИ сегмент `claude-haiku` в ID/ARN. Подстрока `haiku` где угодно отвергнута: FP на кастомных именах деплоя не-Anthropic провайдеров (`prod-haiku-router`, переименованный на Sonnet-класс). Замер 2026-07-27 по транскриптам: правило соблюдалось 10/12, два прогона Клод явно запустил на haiku вопреки SKILL.md — текстом это не лечится (стиль репо: давление несёт хук, не тон). Эхо модели в reason — `sanitize(trunc(…, 60))`; тест на ANSI проверяет распарсенный `reason`, а НЕ stdout: в JSON ESC экранирован, наивная проверка stdout — пустышка (поймано мутацией).
- **`safeInputStr` вместо `String(inp.X || "")`** (`checks.js`, все четыре поля `findReviewAgentCalls`): значение из транскрипта — недоверенный JSON, и `String({"toString": 1})` БРОСАЕТ TypeError (ToPrimitive не находит callable). Необёрнутый throw = silent exit всего Stop-хука, то есть отключение всех триггеров A–N одной записью в транскрипте. Не-строки → `""`, длина капится. Вызов `findReviewAgentCalls` в `verify-changes.js` дополнительно под `try/catch` с fail toward требования (аномалия → секции считаются незапущенными, а не «всё ок»). Класс предсуществовал у `subagent_type`/`description`/`prompt` — закрыт разом.
- **Known gaps модельного гейта** (все — в сторону молчания зуба; ловится только явный haiku в самом вызове): модель переопределяют `CLAUDE_CODE_SUBAGENT_MODEL` (приоритет выше per-invocation) и организационный `availableModels`-allowlist — реально исполненная модель может отличаться от записанной в обе стороны, поэтому reason это оговаривает; `model` в frontmatter кастомного сабагента хуку не виден (`input.model` пуст); haiku-сессия целиком по бюджету без явного override — структурно не ловится (осознанно: иначе ложный блок именно у бюджетного юзера); декоративный второй premortem-вызов без `model` после реального haiku-прогона сбрасывает `edgeModel` — тот же потолок текстового матча, что у M. Ремедиация после блока — повторный сабагент-вызов, а Edit'а нет, поэтому анти-луп-гард `lastBlockIdx > lastEditIdx` снимает блок на следующем Stop независимо от переанонса: «слово юзеру» by design, как у fake-decl.
