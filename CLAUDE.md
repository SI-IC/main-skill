...
- **Декларативные ORM-модели** (`isDeclarativeModelFile` в `checks.js`, `.ts/.js`): skip, если гейт (класс `extends *BaseModel` — Lucid, суффикс-конвенция пропускает кастомную базу `AppBaseModel`; либо `@Entity(` в НАЧАЛЕ строки + класс — TypeORM, якорь режет `@Entity` в строковом литерале; `extends compose(...)` гейт НЕ проходит — миксин несёт поведение) И есть хоть одно column/relation/`declare`-поле И ноль сигналов логики после stripBlockComments. Relation-thunk-стрелки нейтрализуются ТОЛЬКО в аргументной позиции (после `(`/`,`/`[`): `() => Model` и inverse-side `(x) => x.prop` (цепочка без вызова, позитивный lookahead на разделитель `)`/`,`/`]`/перенос — частичный ретрит идентификатора не проходит). Любая другая стрелка (значение `= () =>`, опция `serialize:`/`onQuery:`), тело метода — имя-агностичный сигнал `){` (ловит многострочные сигнатуры и computed/unicode/`#`-имена), `static {}`-блок, `get`/`set`, `@computed`, hook (`@beforeSave`/`@BeforeInsert`/…), `serializeExtras`, `scope(`, `return`/`this`/`await`/`new`, присваивание вызова `= f(...)` → НЕ skip. Компромиссы как у SFC — FP безопаснее FN, снимается узким env-игнором: TypeORM embedded-классы (без `@Entity` by design) и базы без суффикса `BaseModel` требуют лишний тест; `static x = new ...` — тоже. Известный FN — логика в импортированном хелпере внутри опций декоратора (`onQuery: helperFn`): её тестирует парный тест самого хелпера. Кейс ERP_NEW: 106 чистых моделей в `apps/api/app/models/` перестают требовать каталожный ignore-глоб.
