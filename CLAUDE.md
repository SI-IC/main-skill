...
- **Порядок чтения — по релевантности, не по алфавиту** (`rankSpecCandidates`): имя источника в basename спека +4 (для generic — имя родителя), родительский сегмент в пути +2, токен basename +1; гейт длины ≥3 (короткое `db` ⊂ `redblue` — шум), tie → алфавит. Ошибка скоринга меняет лишь порядок чтения, не результат; покрытый файл находится за единицы чтений — кэп перестаёт отрезать алфавитно-хвостовые спеки (баг-репорт: пакет >200 спеков → ложный D на покрытом файле).
...
- **Обрыв не тихий**: `cache.lastTruncated` (валиден при null-результате) = бюджет чтений выбран ИЛИ обрезан сам список кандидатов (`collectCentralSpecFiles` возвращает `{files, truncated}`; ревью-регресс: при env-cap ≥ 400 maxList == бюджету — цикл чтения дочитывает обрезанный список штатно и обрыва не видит). verify-changes собирает `truncatedScans`, reasonD получает ⚠-блок «покрытие НЕ подтверждено» с grep-рецептом и ручкой лимита; рецепт — `grep -rlF` + allowlist `[A-Za-z0-9._-]` на basename (недоверенное имя `bil$(id)ling.ts` пронесло бы `$(id)` в шелл — reason подталкивает Клода выполнить команду; sanitize стрипует только controls/BiDi). D всё равно срабатывает (fail toward требования) — но FP от кэпа больше не выглядит как факт «теста нет», и Claude не толкается к дубль-тестам/глобам.
